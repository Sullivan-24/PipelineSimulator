...
def draw_strong_scaling_bar():
    ...
    data_self = np.array([[strong_scaling_data[seq][method] / strong_scaling_data["GPU#=8"][method] for method in methods] for seq in x])
    ...
    for i, method in enumerate(methods):
        ...
        for bar, y in zip(bars, y_vals):
            ...
            # plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
            #          f"{y:.2f}x", ha="center", va="bottom", rotation=90, color=line_colors[method], fontsize=ticksize)
    # 坐标轴和标签
    plt.xticks(x_pos + bar_width * (len(methods) - 1) / 2, x, fontsize=ticksize)
...
def draw_weak_scaling_bar():
    ...
    data_self = np.array([[weak_scaling_data[seq][method] / weak_scaling_data["GPU#=8"][method] for method in methods] for seq in x])
