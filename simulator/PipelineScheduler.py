...
class PipelineSchedulerSolver:
    ...
    def __init__(self, config: dict, device_layer_alignments=None, new_comm_length=None) -> None:
        ...
        self._profiled_comm_length = config["communication_time"] if not new_comm_length else new_comm_length
        ...
        # 创建 Gurobi 模型
        self.model = Model("PipelineSchedulerSolver")
    ...
    def _build_constraints(self) -> None:
        ...
        # 添加约束
        self._profiled_comm_length = self._reset_comm_length(self._devices)
        ...
        for mb in range(self._num_microbatches):
            for i in range(1, self._num_layer):
                self.model.addConstr(self._layer_f_offsets[i][mb] >= self._layer_f_offsets[i - 1][mb] +
                                     self._layer_f_length[i - 1] + self._profiled_comm_length[i - 1][i])

            for i in range(self._num_layer - 1, 1, -1):
                self.model.addConstr(self._layer_b_offsets[i - 1][mb] >= self._layer_b_offsets[i][mb] +
                                     self._layer_b_length[i] + self._profiled_comm_length[i][i - 1])
    ...
    def _draw(self, results: dict) -> None:
        ...
        painter_conf = {
            ...
            "comm_length": self._profiled_comm_length,
