        ...
        for did in range(self._num_device):
            for lid in self._devices[did]:
                for mid in range(self._num_microbatches):
                    ...
                    base_memory = (OPTIMIZER_MEMORY / (PP_SIZE * TP_SIZE)) + ModelState.LAYER * len([l for l in self._devices[did] if l not in (0, LAYER_NUM + 1, LAYER_NUM + 2)]) + contain_head * HEAD_MEMORY
