...
def set_execution_time(time_value, stages_num = None):
    global forward_execution_time, backward_execution_time, backward_execution_time2
    forward_execution_time = [time_value for _ in range(stages_num)] if stages_num else [time_value for _ in range(len(forward_execution_time))]
    backward_execution_time = [t * 1.5 for t in forward_execution_time]
    backward_execution_time2 = [t * 1 for t in forward_execution_time]
...
def main():
    ...
    config = {
        ...
        "virtual_stage": (2, [1, 0]),
        "num_microbatches": int(len(forward_execution_time) * 1),
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        set_execution_time(eval(sys.argv[-1]))
    elif len(sys.argv) == 3:
        set_execution_time(eval(sys.argv[-2]), int(sys.argv[-1]))
