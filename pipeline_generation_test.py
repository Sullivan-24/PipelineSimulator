...
def clear_old_files():
    ...
    for did in range(DEVICE_NUM):
        workload_stat_filepath = f"schedule_results/{MODEL_NAME}/workload_statistics/device{did}.txt"
        clear_old_file(workload_stat_filepath)
        memory_record_filepath = f"schedule_results/{MODEL_NAME}/memory/device{did}.txt"
